...
GPU_NUM=0
MODEL_ID=bksdm-v2-base-212k-ckpt_50k-3w

IMG_PATH=./outputs/$MODEL_ID/im256

echo "=== Inception Score (IS) ==="
IS_TXT=./outputs/$MODEL_ID/im256_is.txt
fidelity --gpu $GPU_NUM --isc --input1 $IMG_PATH | tee $IS_TXT
echo "============"

echo "=== Fréchet Inception Distance (FID) ==="
FID_TXT=./outputs/$MODEL_ID/im256_fid.txt
NPZ_NAME_gen=./outputs/$MODEL_ID/im256_fid.npz
NPZ_NAME_real=/home/wanghesong/Datasets/mscoco_val2014_41k_full/real_im256.npz
...
echo "=== CLIP Score ==="
CLIP_TXT=./outputs/$MODEL_ID/im256_clip.txt
